package com.example.transactionscript.script;
...
import com.example.transactionscript.gateway.ContractTableDataGateway;
import com.example.transactionscript.gateway.ProductTableDataGateway;
import com.example.transactionscript.gateway.RevenueRecognitionTableDataGateway;
import com.example.transactionscript.helper.RevenueRecognitionFactory;
...
public class RevenueRecognitionScriptImpl implements RevenueRecognitionScript {
	...
	public RevenueRecognitionScriptImpl(RevenueRecognitionTableDataGateway recognitionGateway, ContractTableDataGateway contractGateway, ProductTableDataGateway productGateway) {
		super();
	...
	public int insertContractInformation(int productId, MonetaryAmount contractPrice, LocalDate dateSigned) {
		return contractGateway.insert(productId, contractPrice.getNumber().numberValue(BigDecimal.class).setScale(contractPrice.getCurrency().getDefaultFractionDigits(), RoundingMode.HALF_EVEN), dateSigned);
	}
